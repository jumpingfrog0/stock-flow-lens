...
风格、同行、评分和置信度阈值只在后端归因引擎中定义，并通过 API 的 `methodologyVersion`
标识版本。文档和 Skill 不复制数值阈值。行业关键词只用于启发式风格分类，不是正式指数归属。
...
资金流模块和归因模块共用 `app.infrastructure.eastmoney` 中的请求、重试和域名降级能力；
归因模块额外允许在 Python HTTP 连接被上游主动断开时降级到系统 `curl` 传输。
